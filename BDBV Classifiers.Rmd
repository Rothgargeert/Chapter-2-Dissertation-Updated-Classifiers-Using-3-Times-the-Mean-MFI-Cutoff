...
```{r}
BDBVimp<-df7[, c('BDBV', 'LLOV','TAFV', 'SUDV',
'EBOV','Village','MENV')]
```
```{r}
library(caret)
...
```{r}
cmNaive <-confusionMatrix(NaivePredictions, ValidSet$BDBV, positive='Positive')
```
...
cmNN <-confusionMatrix(NNPredictions, ValidSet$BDBV, positive='Positive' )
```
...
cmTree <-confusionMatrix(DTPredictions, ValidSet$BDBV, positive='Positive')
```
