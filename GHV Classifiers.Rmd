...
```{r}
GHVimp<-df7[, c('GHV','MENV','CEDV','HEV')]
```
...
```{r}
cmNaive <-confusionMatrix(NaivePredictions, ValidSet$GHV, positive='Positive' )
```
```{r}
cmNaive
```
...
cmNN <-confusionMatrix(NNPredictions, ValidSet$GHV, positive='Positive')
```
...
cmTree <-confusionMatrix(DTPredictions, ValidSet$GHV, positive='Positive')
```
