...
```{r}
MENVimp<-df7[, c('MENV','GHV','Village', 'RAVV','TAFV',
'Pig')]
```
...
```{r}
cmNaive <-confusionMatrix(NaivePredictions, ValidSet$MENV, positive='Positive' )
```
```{r}
cmNaive
```
...
cmNN <-confusionMatrix(NNPredictions, ValidSet$MENV, positive='Positive')
```
...
cmTree <-confusionMatrix(DTPredictions, ValidSet$MENV, positive='Positive')
```
