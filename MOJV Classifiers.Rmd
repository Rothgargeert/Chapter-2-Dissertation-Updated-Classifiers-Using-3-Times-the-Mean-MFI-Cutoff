...
```{r}
MOJVimp<-df7[, c('MOJV','Pig','Duck','Cat', 'Village','HH.46.to.60')]
```
...
```{r}
cmNaive <-confusionMatrix(NaivePredictions, ValidSet$MOJV, positive='Positive' )
```
```{r}
cmNaive
```
...
cmNN <-confusionMatrix(NNPredictions, ValidSet$MOJV, positive='Positive')
```
...
cmTree <-confusionMatrix(DTPredictions, ValidSet$MOJV, positive='Positive')
```
