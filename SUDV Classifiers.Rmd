...
```{r}
SUDVimp<-df7[, c('SUDV','EBOV','LLOV','TAFV', 'BDBV','Cat',
'Pig')]
```
...
```{r}
cmNaive <-confusionMatrix(NaivePredictions, ValidSet$SUDV, positive='Positive' )
```
```{r}
cmNaive
```
...
cmNN <-confusionMatrix(NNPredictions, ValidSet$SUDV, positive='Positive')
```
...
cmTree <-confusionMatrix(DTPredictions, ValidSet$SUDV, positive='Positive')
```
